...
from kvfleet.cache.fingerprints import PromptFingerprinter, PromptFingerprint, _normalize_text
from kvfleet.cache.kv_affinity import ConsistentHashRing, SessionAffinityStore, KVAffinityScorer
...
class TestKVAffinityScorer:
    def test_score_affinity(self):
        ...
        fp = PromptFingerprint(full_hash="abc", system_hash="sys", prefix_hash="pfx", conversation_hash="conv")
    ...
    def test_record_and_affinity(self):
        scorer = KVAffinityScorer()
        fp = PromptFingerprint(full_hash="h1", system_hash="s1", prefix_hash="p1", conversation_hash="c1")
    ...
    def test_best_endpoint(self):
        scorer = KVAffinityScorer()
        fp = PromptFingerprint(full_hash="h", system_hash="s", prefix_hash="p", conversation_hash="c")
        scorer.record_routing(fp, "http://a:8000")
...
class TestSemanticCache:
    def test_put_get(self):
        cache = SemanticCache()
        fp = PromptFingerprint(full_hash="h1", system_hash="s", prefix_hash="p", conversation_hash="c")
    ...
    def test_miss(self):
        cache = SemanticCache()
        fp = PromptFingerprint(full_hash="miss", system_hash="s", prefix_hash="p", conversation_hash="c")
    ...
    def test_eviction(self):
        ...
        for i in range(5):
            fp = PromptFingerprint(full_hash=f"h{i}", system_hash=f"s{i}", prefix_hash=f"p{i}", conversation_hash=f"c{i}")
